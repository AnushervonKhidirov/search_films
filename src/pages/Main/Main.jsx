...
function Main() {
    ...
    let url = `https://api.themoviedb.org/3/movie/popular?api_key=${apiKye}&language=en-US&page=${page}`;


    useEffect(() => {
        if (searchParams.get('movie')) {
            url = `https://api.themoviedb.org/3/search/movie?api_key=${apiKye}&language=en-US&query=${searchParams.get('movie')}&page=${page}`
        };

        fetch(url)
            .then(response => response.json())
            .then(film => setFilms(film.results));
    }, [url, apiKye, searchParams]);
